...
class ObjectDetectionCamera(Camera):
    """
    Camera specialized for YOLO object detection with RKNN support.
    Extends base Camera with:
    - Model loading and preprocessing
    - Object detection pipeline
    - Coordinate transformations (pixel to robot-relative)
    """
    def __init__(
        self,
        camera_config,
        ...
        # ===== YOLO/RKNN Vision Model Configuration =====
        ...
        # ===== RKNN preprocessing pipeline (if needed) =====
    ...
    def _preprocess_worker(self):
        """Background thread for RKNN preprocessing pipeline."""
        last_ts = None
    ...
    def _letterbox_into(self, img, dst, target_size):
        """Resize image with letterboxing into destination buffer."""
        h, w = img.shape[:2]
    ...
    def _letterbox(self, img, target_size=(640, 640)):
        """Resize image with letterboxing (returns new image)."""
        h, w = img.shape[:2]
        target_w, target_h = target_size
        scale = min(target_w / w, target_h / h)
    ...
    def _filter_box(self, box: Box, img_w: int, img_h: int) -> bool:
        """Filter detection boxes by confidence, margins, and aspect ratio."""
    ...
    def _box_to_robot_point(self, box: Box, img_w: int, img_h: int) -> np.ndarray | None:
        """Convert detection box to robot-relative coordinates."""
        x1, y1, x2, y2 = box.xyxy
        w_px = x2 - x1
        h_px = y2 - y1
    ...
    def get_yolo_data(self):
        """Get YOLO detection results and annotated frame."""
    ...
    def run_with_supplied_data(self, data):
        """Process supplied detection results into map points."""
    ...
    def run(self):
        """Main detection pipeline: get detections and convert to robot-relative coordinates."""
    ...
    def get_data_for_subsystem(self, target: str):
        """Get processed data for a specific subsystem target."""
    ...
    def _pixel_to_robot_coordinates(self, pixel_x, pixel_y, distance_los, img_w, img_h):
        """Transform pixel coordinates to robot-relative world coordinates."""
    ...
    def get_subsystem(self):
        """Get the subsystem this camera is associated with."""
